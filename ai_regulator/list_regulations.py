...
改定の判断基準: 
1. 情報の更新：既存の規定内に関連する記述がある場合のみ、改定が必要と判断してください。
   例：手数料率が年1.5%から年2.0%に変更される場合、既存の手数料率記載箇所の更新のみ必要。

2. 情報の追加：既存の規定内に関連する記述が全くない場合のみ、改定が必要と判断してください。
   例：これまで手数料率の定めがなかった取引に新たに手数料率が設定される場合のみ、改定が必要。

3. 情報の削除：変更情報に基づき、既存のルールが廃止される場合のみ、改定が必要と判断してください。
   例：特定の手続き方法が廃止される場合、その手続きに関する記述の削除が必要。

4. 無関係な追加の禁止：変更情報に直接関係のない箇所は、たとえ改善の余地があっても改定不要と判断してください。
...
2. 情報の追加：既存の規定内に関連する記述が全くない場合のみ改定が必要
3. 情報の削除：変更情報に基づき、既存のルールが廃止される場合のみ改定が必要
4. 無関係な追加の禁止：変更情報に直接関係のない箇所は改定不要
