...
1. フォーマット評価(Format Check)：改定案は規定集の形式と記載原則を適切に維持しているか確認して建設的なフィードバックを提供してください
   - 文書構造は一貫していますか？
   - 用語の使用は統一されていますか？
   - 規定特有の表現や形式は保持されていますか？
   - 「一部のみ変更」「当面の間」「なお～のままです」など、変更過程や状態の一時性を説明する不要な文言が含まれていませんか？
   - 移行期間や変更に関する説明が規定本文に含まれていませんか？
   - 現在の状態のみを記述し、変更過程の説明を避けていますか？
...
3. 一貫性評価(Consistency Check)：規定集の役割・目的との整合性と改定内容の判断基準を確認して建設的なフィードバックを提供してください
   - 規定の本来の意図は保持されていますか？
   - 元の規定文にない情報を不必要に追加していませんか？
   - 規定は現在の状態のみを記述し、変更過程や移行期間に関する説明を避けていますか？
   - 既存の規定内に関連する記述がある場合、その部分のみを更新していますか？
   - 新しい情報の追加は、既存の規定内に関連する記述が全くない場合のみに限定されていますか？
   - 変更情報に基づかない情報が追加されていませんか？
   - ルール廃止の場合、該当する記述は適切に削除されていますか？
